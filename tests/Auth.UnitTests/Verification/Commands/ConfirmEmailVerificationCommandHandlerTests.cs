...
using Auth.Infrastructure.Verification.Commands.ConfirmEmailVerification;
...
{
    private static ConfirmEmailVerificationCommandHandler CreateHandler(AuthDbContext dbContext)
    {
        return new ConfirmEmailVerificationCommandHandler(
            dbContext,
            new Mock<IAuditContext>().Object,
            new Mock<ILogger<ConfirmEmailVerificationCommandHandler>>().Object);
    }
    ...
    [Fact]
    public async Task Confirm_WhenValid_SetsEmailVerified()
    {
        await using var dbContext = CreateDbContext();
        var user = new User
        {
