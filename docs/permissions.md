...
Для каждого доступного workspace в токене создаётся отдельный claim `ws:<code>`:

```json
{
  "ws:system": {"system": "//8f8A=="},
  "ws:dev": {"system": "AgA="}
}
```

Значение claim — JSON-объект `{domain: base64-bitmask}`.
...
Проверка полномочий выполняется в `PermissionInHandler` без обращения к БД: по claim `ws:<workspaceCode>` находится словарь доменов, по домену — bitmask, в котором проверяется нужный бит.

## Wildcard scope `ws:*`

Scope `ws:*` позволяет клиенту запросить все доступные workspace без перечисления кодов. В токен попадают только workspace, к которым у субъекта (пользователь/SA) есть доступ — `ws:*` не расширяет права.

Настройка: admin добавляет `ws:*` в `Application.Scopes`. Если приложению не выдан `ws:*`, OpenIddict отклонит запрос.

    scope=openid profile ws:*

Можно комбинировать с явными скоупами: `ws:* ws:system` — wildcard имеет приоритет, результат тот же что и `ws:*`.
